...
* **Transactions** : Les sauvegardes de coups se font via `SaveMovesBulk` pour optimiser les performances.
* **Nettoyage** : La méthode `DeleteGame` supprime en cascade les coups associés.
* **Table `claimed_spots`** : Créée automatiquement dans `EnsureColumnsExist`. Les méthodes `SaveClaimedSpots` et `GetClaimedSpots` gèrent la persistance des revendications.
